...
- 네트워크는 3개의 FCL 블럭으로 이루어져 있다. `The network has three blocks of fully convolutional layers. `
	- The first layer of each block down samples the feature map by half via a convolutionwith a stride size of 2, followed by a sequence of convolutionsof stride 1 (×q means q applications of the filter).
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIxMzQ4NjE4MTldfQ==
-->