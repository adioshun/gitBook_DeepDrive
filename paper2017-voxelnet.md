...
- 다음, centroid와 입력 특징셋 $$V_{in}포인트를 증폭시킨다. Then we augment each point $$p_i$$ with the relative offset w.r.t. the centroid and obtain the input feature set $ V_{in} = \{\hat p_i = \[x_i, y_i, z_i, r_i, x_i − v_x, y_i −v_y, z_i −v_z\]^T \in \Re^7\}_{i=1...t}$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEyNjI0NDc3OTNdfQ==
-->