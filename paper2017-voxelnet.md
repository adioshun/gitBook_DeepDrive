...
- `point-wise feature representations`를 획득후 $V$에 연관된 모든 $$f_i$$에 대하여 `element-wise MaxPooling`를 적After obtaining point-wise feature representations, we use element-wise MaxPooling across all $$f_i$$ associated to $V$ to get the locally aggregated feature $$\tilde{f} \in \Re^m$$ for V. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwOTY3ODE4NjhdfQ==
-->